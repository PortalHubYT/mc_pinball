...
class Component(ApplicationSession):
    ...
    async def check_alive(self):
        ...
        for i in names_copy:
            ...
            if not isinstance(y_pos, int):
                ...
                if y_pos <= 5:
                    if z_pos - self.gamestate["origin_z"] <= int(
                        self.gamestate["width"] / 5
                    ):
                        print(
                            f"respawning le pote: {i} {names_copy[str(i)]}, z= {z_pos}"
                        )
                        self.call(
                            "minecraft.post",
                            f"execute as @e[tag=ball, tag={i}] at @s run tp {(self.gamestate['origin_x'] + self.gamestate['depth']) / 2} ~{self.gamestate['height'] + self.gamestate['origin_y'] - 3} {random.randrange(self.gamestate['origin_z'], self.gamestate['width'])}",
                        )
    ...
    async def display_scores(self, names):
        top_ten = sorted(self.scores.items(), key=lambda item: item[1], reverse=True)[
            :10
        ]
        current_glowing = 0
    ...
    def delete_player(self, uid, name):
        ...
        self.call(
            "minecraft.post",
            f"scoreboard players reset {name[:SCOREBOARD_WIDTH]} alive_for",
        )
        ...
        if score > self.highscore and not "Nightbot" in name:
